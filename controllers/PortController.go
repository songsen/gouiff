...
func (this *PortController) Post(){
		...
		case "updatelinebyfeild":
			...
			this.PublicMessage(fmt.Sprint("更新了!!! %s ",this.feild))
		case "deleteline" :
			o := orm.NewOrm()
			num,_ := o.Delete(&this.Shippingline)
			this.Ctx.WriteString(fmt.Sprintf("影响行数：%d",num))
			this.PublicMessage(fmt.Sprint("删除了!!! "))
		case "addoneline" :
			...
			this.PublicMessage(fmt.Sprint("添加了!!! "))
		}
	}
...
func (this *PortController) Get(){ //query line
	...
	if ok {
		...
		switch this.msgType{
		case "fromport":
			tips := this.getPort("from")
			sort.Strings(*tips)
			*tips = Duplicate(*tips)
			this.Ctx.WriteString(strings.Join(*tips, ","))
		case "toport":
			tips := this.getPort("to")
			sort.Strings(*tips)
			*tips = Duplicate(*tips)
			this.Ctx.WriteString(strings.Join(*tips, ","))
		// case "region":
...
func (this *PortController) getPort(flag string) *[]string{
